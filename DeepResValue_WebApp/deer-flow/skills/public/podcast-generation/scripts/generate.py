...
def text_to_speech(text: str, voice_type: str) -> Optional[bytes]:
    ...
    app_id = os.getenv("ALIYUN_TTS_APPKEY")
    access_token = os.getenv("ALIYUN_TTS_ACCESS_TOKEN")
    cluster = os.getenv("ALIYUN_TTS_CLUSTER", "volcano_tts")

    if not app_id or not access_token:
        raise ValueError(
            "ALIYUN_TTS_APPKEY and ALIYUN_TTS_ACCESS_TOKEN environment variables must be set"
        )
...
def tts_node(script: Script, max_workers: int = 4) -> list[bytes]:
    ...
    if not os.getenv("ALIYUN_TTS_APPKEY") or not os.getenv("ALIYUN_TTS_ACCESS_TOKEN"):
        raise ValueError(
            "Missing required environment variables: ALIYUN_TTS_APPKEY and ALIYUN_TTS_ACCESS_TOKEN must be set"
        )
    ...
    if not audio_chunks:
        raise ValueError(
            ...
            "Please check ALIYUN_TTS_APPKEY and ALIYUN_TTS_ACCESS_TOKEN environment variables."
        )
