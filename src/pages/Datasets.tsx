...
import { ArrowLeft, Database, Upload, FileText, MoreVertical, Trash2, Library, Zap } from "lucide-react";
import { useStore } from "@/store/useStore";
import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";

const BUILT_IN_DATASETS = [
  { id: 'b1', name: '上市企业财务指标面板数据 (2010-2023)', type: '微观企业', rows: '2.4M', points: 50 },
  { id: 'b2', name: '全国县域宏观经济统计数据 (2000-2022)', type: '宏观县域', rows: '450K', points: 30 },
  { id: 'b3', name: '全国地级市统计年鉴面板数据 (2000-2023)', type: '宏观市级', rows: '85K', points: 20 },
  { id: 'b4', name: '各省份宏观经济及创新指数面板数据 (2005-2023)', type: '宏观省级', rows: '15K', points: 10 },
];
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {BUILT_IN_DATASETS.map((ds) => (
                      <tr key={ds.id} className="hover:bg-amber-50/30 transition-colors group">
                        ...
                        <td className="p-4 text-right">
                          <div className="inline-flex items-center space-x-1 px-2 py-1 bg-amber-50 text-amber-700 rounded-md text-sm font-medium">
                            <Zap className="w-3.5 h-3.5 fill-amber-500 text-amber-500" />
                            <span>{ds.points} 积分 / 次</span>
                          </div>
                        </td>
                      </tr>
                    ))}
