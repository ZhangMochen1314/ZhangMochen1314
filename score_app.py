...
def get_content(idx, folders):
    ...
    academic_img = os.path.join(BASE_DIR, folder, "academic.png")
    commercial_img = os.path.join(BASE_DIR, folder, "commercial.png")
...
def load_initial_state():
    folders = get_folders()
    init_csv()
    progress_text, academic_img, commercial_img = get_content(0, folders)
    return 0, progress_text, academic_img, commercial_img
...
with gr.Blocks(title="图片打分工具") as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            academic_image = gr.Image(label="Academic.png", interactive=False)
            academic_slider = gr.Slider(minimum=0, maximum=100, value=50, step=1, label="Academic Score")
            
        with gr.Column():
            gr.Markdown("### Commercial")
            commercial_image = gr.Image(label="Commercial.png", interactive=False)
    ...
    demo.load(
        ...
        inputs=[],
        outputs=[state_idx, progress_md, academic_image, commercial_image]
    )
