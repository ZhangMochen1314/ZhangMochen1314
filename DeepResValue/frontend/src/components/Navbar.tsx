      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16">
        ...
        <div className="flex items-center space-x-4">
          {isAuthenticated ? (
            ...
            <>
              <button onClick={() => useAuthStore.getState().openAuthModal('login')} className="text-sm font-medium text-slate-600 hover:text-blue-600 transition-colors">
                登录
              </button>
              <button onClick={() => useAuthStore.getState().openAuthModal('register')} className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors shadow-sm">
                免费注册
              </button>
            </>
          )}
