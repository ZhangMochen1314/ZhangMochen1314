...
from app.session_manager import get_session_manager, SessionState
...
async def start_run(
    ...
    # Initialize SessionManager for Session persistence (Task 4.4)
    ...
    try:
        ...
        session_state = await session_mgr.get_session(thread_id, db)
        if not session_state:
            session_state = SessionState(session_id=thread_id, user_id=user_id)
            await session_mgr.save_session(session_state, db)
            await session_mgr.log_operation(thread_id, "session_created", {"assistant_id": body.assistant_id}, db)
        elif session_state.status == "interrupted":
            session_state.status = "active"
            await session_mgr.save_session(session_state, db)
            await session_mgr.log_operation(thread_id, "session_resumed", {"assistant_id": body.assistant_id}, db)
    finally:
        try:
            next(db_gen)
    ...
    def on_run_done(t):
        ...
        db = next(db_gen)
        try:
            # We can't use await here because it's a sync callback for a task, so we should schedule it
            async def cleanup_session():
                try:
                    s_state = await session_mgr.get_session(thread_id, db)
                    if s_state:
                        s_state.status = "interrupted" if t.cancelled() else "completed"
                        await session_mgr.save_session(s_state, db)
                        await session_mgr.log_operation(thread_id, f"session_{s_state.status}", {}, db)
                finally:
                    try:
                        next(db_gen)
                    except StopIteration:
            ...
            logger.error(f"Error in on_run_done: {e}")

    task.add_done_callback(on_run_done)
