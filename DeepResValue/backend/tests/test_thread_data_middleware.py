...
class TestThreadDataMiddleware:
    def test_before_agent_returns_paths_when_thread_id_present_in_context(self, tmp_path):
        ...
        result = middleware.before_agent(state={}, runtime=Runtime(context={"thread_id": "tenant_1-thread-123"}))

        assert result is not None
        assert _as_posix(result["thread_data"]["workspace_path"]).endswith("tenant_1/user-data/workspace")
        assert _as_posix(result["thread_data"]["uploads_path"]).endswith("tenant_1/user-data/uploads")
        assert _as_posix(result["thread_data"]["outputs_path"]).endswith("tenant_1/user-data/outputs")
    ...
    def test_before_agent_uses_thread_id_from_configurable_when_context_is_none(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: {"configurable": {"thread_id": "tenant_1-thread-from-config"}},
        )
