...
async def confirm_uploads(thread_id: str, request: ConfirmRequest) -> UploadResponse:
    ...
    for object_name in request.object_names:
        ...
        try:
            ...
            logger.info(f"Confirmed and downloaded file: {safe_filename} ({len(content)} bytes) to {file_info['path']}")
            ...
            logger.error(f"Failed to process confirmed file {object_name}: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to process {object_name}: {str(e)}")

    return UploadResponse(
        success=True,
        files=uploaded_files,
        message=f"Successfully confirmed {len(uploaded_files)} file(s)",
    )
