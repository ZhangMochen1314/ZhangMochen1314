...
1. **意图澄清与数据预检 (Data Validation & Clarification)**：
   - 空间计量需要强依赖空间权重矩阵。如果用户只上传了数据文件（如 CSV/Excel）而未说明空间关系（例如是否包含经纬度、或者是否需要结合外部 Shapefile/权重矩阵），**必须主动询问**用户：“您的空间权重矩阵打算如何构建？数据中是否包含地理坐标（经纬度），或者您是否能提供邻接矩阵文件？”
   - 必须先查看并分析数据的前几行（通过 `pd.read_csv().head()`），确认因变量 (`y`) 和自变量 (`X`)。如果用户未明确指定模型设定（如因变量和控制变量是什么），**必须主动询问**用户具体的回归公式设定。
2. **任务目标**：处理带有地理或空间信息的数据，计算空间权重矩阵，并执行空间自回归模型(SAR)、空间误差模型(SEM)及空间杜宾模型(SDM)等高级空间计量估计。
