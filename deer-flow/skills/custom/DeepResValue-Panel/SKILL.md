...
1. **意图澄清与数据预检 (Data Validation & Clarification)**：
   - 收到数据文件后，**必须先在沙盒中执行探针脚本**（如 `pd.read_csv().head()` 和 `.info()`）探查数据结构，确认数据是否为面板格式（包含个体标识和时间标识）。
   - 面板回归模型依赖于明确的模型设定。如果用户只说“做个面板回归”，**必须主动询问**并确认以下参数：
     - **因变量 (y)** 和 **自变量 (X)** 分别是什么？
     - **固定效应 (Fixed Effects)** 应该控制哪些维度（如：是否控制个体效应、时间效应，或高维行业/省份效应）？
     - **标准误聚类 (Cluster)** 是否需要聚类到特定层级（如：聚类到个体层面）？
   - 根据数据探针，如果因变量只包含 0 和 1，**必须主动提醒用户**：“您的因变量为二元离散变量，我将为您采用面板 Logit/Probit 模型进行估计。”
2. **任务目标**：处理面板数据，执行带有多维固定效应和多向聚类标准误的回归分析。
